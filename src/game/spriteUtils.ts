...
/**
 * Removes dark/black background connected to image edges.
 * Useful for drawings exported on black canvas.
 */
export function loadSpriteTransparentDark(
  src: string,
  threshold = 28
): Promise<HTMLCanvasElement> {
  return new Promise((resolve, reject) => {
    ...
    img.onload = () => {
      ...
      const isDarkBackground = (idx: number): boolean => {
        const p = idx * 4;
        return d[p] <= threshold && d[p + 1] <= threshold && d[p + 2] <= threshold;
      };
      ...
      const enqueue = (x: number, y: number) => {
        const idx = y * w + x;
        if (visited[idx]) return;
        if (!isDarkBackground(idx)) return;
        visited[idx] = 1;
        queue.push(idx);
      };
